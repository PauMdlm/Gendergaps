...
# Parameters 
ANNEE_COHORTE = 2018 #  2018 2019 2020 2021
...
table(data_depp_1$age_abnormality)                     
nrow(data_depp_1[is.na(data_depp_1$age_abnormality),]) 
...
geom_histogram(binwidth = 1, color = "black", alpha = 0.6) +
 ...
 ggsave(paste0("~/img/",ANNEE_COHORTE,"/age_distribution_cohort_after_outliers_treatment_", ANNEE_COHORTE,"_1.5year.pdf"))
...
# graph box plot for the variable
...
In cases with many 0's and NA's on a series of test on a period, the child is considered absent, the O's and NA's are replaced by NA's on the whole line. This allows to avoid considering that the child had 0 at this exam, but in fact was not present.
...
# Graph Number of Children per school
...
data_depp_imputed$Age_categ <- factor(data_depp_imputed$Age_Cat, levels = c("Young", "Normal" , "Late"))
