...
#### Graph 1C Math x Total age population x Level x 10 categories and at T1, T2, T3 - TOTAL AGE
...
##### Graph 2D : Density per class Math GG : Evolution of the gender gap distribution between T1, T2 and T3 per class

Data selection of classes : total population (too complicated to select on 30% boys/girls)

##### Density per percentage of level per class in math

```{r}

data_for_plot <- joined2[, c( "D_T3_Math", "D_T2_Math", "D_T1_Math", "Sexe_Boys")]
...
ggplot(data_for_plot, aes(x = Grade, fill = Period, group = Period)) +
  ...
  theme_minimal() +
  xlab("Gender gap in math (percentage)") +
  theme(axis.line = element_line(arrow = arrow(length = unit(2, "mm")))) + 
  theme(axis.title.x = element_text(size=9,  color="black", angle=0, hjust = 1)) +
  theme(axis.title.y = element_text(size=9, color="black", angle=0, hjust = 1)) +
...
ggsave(file = paste0("~/img/", ANNEE_COHORTE, "/Graph_2D_Density_D_Math_", ANNEE_COHORTE, "_Percentage.svg"),
       width = 9, height = 5)
...
##### Density per PRank of level per class in math

```{r}

data_for_plot <- joined2[, c( "D_T3_Math_Rank", "D_T2_Math_Rank", "D_T1_Math_Rank", "Sexe_Boys")]
...
ggplot(data_for_plot, aes(x = Grade, fill = Period, group = Period)) +
  ...
  theme_minimal() +
  xlab("Gender gap in math (Percentile ranks)") +
  theme(axis.line = element_line(arrow = arrow(length = unit(2, "mm")))) + 
  theme(axis.title.x = element_text(size=9,  color="black", angle=0, hjust = 1)) +
  theme(axis.title.y = element_text(size=9, color="black", angle=0, hjust = 1)) +
  theme(axis.text.x = element_text(size=8, color="black", angle=0, hjust = 1)) +
...
ggsave(file = paste0("~/img/", ANNEE_COHORTE, "/Graph_2D_Density_D_Math_", ANNEE_COHORTE, "_PRanks.svg"),
       width = 9, height = 5)
...
##### 3B Gender gap : class level in math at T1 = T1_Math_class_D
...
##### 3B Gender gap : heterogeneity in math at T1
...
##### 3B Gender gap : boys girls ratio boy_prop_D
...
First of class Math WITHOUT 1st of class, Gau_n, normal age
...
load("~/Desktop/education/ ARTICLE_Final_script/data/cohort_2018_imputed_ModRegLin_Gau_From_joined_V27102023.RData")
...
DATA_Boys   <- Joined_Fin_3[Joined_Fin_3$First_sexe_in_T1_Math > 0 , ]
DATA_Girls  <- Joined_Fin_3[Joined_Fin_3$First_sexe_in_T1_Math < 0 , ]
DATA_Mixed  <- Joined_Fin_3[Joined_Fin_3$First_sexe_in_T1_Math == 0 , ]
sum(is.na(Joined_Fin_3$First_sexe_in_T1_Math)) # 0 NA
...
##### Fig 3C Common on Data Girls Data30n
...
##### Fig 3C Common Mixed on Data_30n
